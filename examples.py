...
def stat_qq():
    def stat(plot: Plot) -> Plot:
        ...
        ht = (
            sample._indices.source
            .select(p_value=sample)
            .key_by()
            .select('p_value')
            .key_by('p_value')
            .persist()
        ...
        def transformation(chart: alt.Chart) -> alt.Chart:
            return chart.encode(
                x=alt.X(
                    "x",
                    axis=alt.Axis(title="Expected -log10(p)"),
                    scale=alt.Scale(domain=[0, 10])
                ),
                y=alt.Y(
                    "y",
                    axis=alt.Axis(title="Observed -log10(p)"),
                    scale=alt.Scale(domain=[0, 10])
                )
            ).mark_point()
...
mt = mt.annotate_cols(
    **(
        hl
        .import_table('data/1kg_annotations.txt', impute=True)
        .key_by('Sample')[mt.s]
    )
)
mt = hl.sample_qc(mt)
mt = mt.filter_cols(
    (mt.sample_qc.dp_stats.mean >= 4)
